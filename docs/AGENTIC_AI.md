...
| **Best Practice Skill Generator** | `.github/agents/best-practice-skill-generator.agent.md` | Generates installable Copilot skills containing best-practice guidance for Canopy components, foundations, and utilities. Reads the `guide.mdx` files, stories, and documentation images, then writes a `SKILL.md` to `skills/best-practice/<name>/`. Also updates `docs/COPILOT_SKILLS.md`. | Open Copilot Chat in your IDE, select the **Best Practice Skill Generator** agent, and provide a component name, category, or `"all"` |
...
**Best Practice Skill Generator — IDE / local:**
Open Copilot Chat in your IDE, select the **Best Practice Skill Generator** agent from the dropdown, and ask it to generate a skill for the component(s) you need:
> "Generate best practice skills for the skeleton component."
...
Consumer migration and best-practice skills are documented in [`docs/COPILOT_SKILLS.md`](COPILOT_SKILLS.md) and generated automatically by the **Migration Skill Generator** and **Best Practice Skill Generator** agents respectively.
...
| `skills/best-practice/{skill-name}/SKILL.md` | Consumer **best-practice skills** generated by the Best Practice Skill Generator agent |

> **Note:** Consumer migration skills (e.g. `canopy-v22-migration`) live in `skills/` and consumer best-practice skills (e.g. `skeleton`) live in `skills/best-practice/`. Both are committed directly to the repo by their respective generator agents. They are **not** listed in `skills-lock.json`. Only skills installed from external repositories (under `.agents/skills/`) appear in `skills-lock.json`.
...
Internal skills live in `.github/skills/` and are available to the Canopy team in their IDE automatically — no installation required.

| Skill | File | Description | Trigger |
|---|---|---|---|
| **review-pr-feedback** | `.github/skills/review-pr-feedback/SKILL.md` | Fetches unresolved inline review comments from a GitHub PR using the GH CLI, pre-investigates each one by reading the referenced files and diff, produces a structured summary, then offers to fix or investigate further. | "review PR comments", "check comments on PR #N", "fetch PR feedback", `/review-pr-feedback` |

### Adding a new internal skill

1. Create a `SKILL.md` file under `.github/skills/{skill-name}/SKILL.md`.
2. Add YAML frontmatter with at minimum a `name` and `description`.
3. Write the skill's instructions in the Markdown body.
4. Add a row to the table above.
