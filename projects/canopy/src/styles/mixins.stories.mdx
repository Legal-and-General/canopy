...
<Meta title="Style/Mixins" parameters={{ viewMode: 'docs', previewTabs: { canvas: { hidden: true } } }} />
...
## lg-focus-outline($bg-color: var(--default-focus-color), $inner-bg-color: var(--default-inner-focus-color))
`$bg-color`: the outer colour of the box-shadow.

`$inner-bg-color`: the inner colour of the box-shadow.
