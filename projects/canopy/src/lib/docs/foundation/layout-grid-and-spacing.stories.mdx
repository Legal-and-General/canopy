...
<Meta title="Foundation/Layout, grid and spacing" parameters={{ viewMode: 'docs', previewTabs: { canvas: { hidden: true } } }} />
...
The grid system is useful for responsive page layouts, we recommend that it is used sparingly, for the majority of cases you may be better off just using flexbox.
...
- A set of [Angular directives](./?path=/docs/directives-grid--grid) are also supplied which can be used to add grid classes to native HTML and Canopy elements
