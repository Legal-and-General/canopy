...
To make web content and web applications more accessible to people with disabilities you can use WAI-ARIA, the Accessible Rich Internet Applications Suite. It especially helps with dynamic content and advanced user interface controls developed with Ajax, HTML, JavaScript, and related technologies.

**Components from the Canopy Angular library have been built to be accessible.**

### Defining the language of the document

Telling the browser which language you are using in your document has many benefits. It's good for SEO and it helps third-party translation tools and browsers to identify the right language and dictionary. Defining the correct language in an HTML page helps assistive technology to choose the correct voice profile or character set.
...
When keyboard-only users interact with your site they use the tab key to jump from link to link. If you have a lot of links in your header or in a menu, they must tab through those every time they come to a new page just to get to the main content. Providing a skip to main content link allows them to easily bypass this.

You can do that by using the class `skip-content`:
...
If an image is used as content it will need the `alt` attribute to describe the images content. If the image is purely decorative or doesn't add any valuable information you can either embed it as a background-image through CSS or setting the `alt` attribute to an empty string:
