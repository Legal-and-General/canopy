...
This component uses some extra card components, such as `LgDataPointComponent` with `variant="card-principle"`, `LgDataPointLabelComponent`, `LgDataPointValueComponent`, and `LgDataPointSecondaryLabelComponent` to display data points.
