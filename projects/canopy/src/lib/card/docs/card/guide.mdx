...
This component uses some extra card components, such as `LgDataPointComponent` with `variant="card-principle"`, `LgDataPointLabelComponent`, `LgDataPointValueComponent`, and `LgDataPointSecondaryLabelComponent` to display a prominent principle data point.

<Source of={CardStories.ProductCard} />

### Image card

This card example demonstrates different variants (default, promo, interactive) with media support. Use the controls to switch between image and brand icon media, and adjust the layout orientation.

<Source of={CardStories.Promo} />
