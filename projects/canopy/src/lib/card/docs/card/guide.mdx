...
When used with `LgOrientationDirective`, cards support `vertical` and `horizontal` layouts only. `reverse-vertical` and `reverse-horizontal` are not supported for cards.
...
Cards are structured into three layout areas:

1. **Header** - optional static or interactive header content.
2. **Content** - the main body where text, images, and brand icons can be combined.
3. **Footer** - action area for links, buttons, or menus.
...
The examples show the `default`, `promo`, and `interactive` card variants.

Example below is a `promo` card variant with Pictogram.
...
<div lgContainer>
  <div lgRow>
    <div lgCol="8" lgColMd="10" lgColSm="12">
      ...
      >
        <lg-card-hero-img [cover]="true">
          <lg-pictogram [name]="'looking-ahead'" size="lg" [hasFill]="false"></lg-pictogram>
        </lg-card-hero-img>

        <lg-card-header>
          <lg-card-title [headingLevel]="2">The title</lg-card-title>
        </lg-card-header>
...
This example demonstrates image-based promo cards in a three-card layout. Use the controls to switch between image and brand icon media, adjust the layout orientation, and change the footer content.
  ...
  <div lgRow>
    <div lgCol="12" lgColMd="4">
      <lg-card variant="promo" [lgOrientation]="{ sm: 'vertical', md: 'horizontal', lg: 'horizontal' }">
        <lg-card-hero-img [cover]="true" [src]="'promo-card/general-promotion1.jpg'" alt="Family sitting in a park"></lg-card-hero-img>
        <lg-card-header>
          <lg-card-title [headingLevel]="2">The title</lg-card-title>
        </lg-card-header>
        <lg-card-content>
...
|----------------|-----------------------------------------------------------------|-------------------------|-----------|----------|
|   `variant`    | The visual variant of the card: `default`, `promo`, or `interactive` | `CardVariant`           | `default` | No       |
...
This is the primary layout section of the card component. It can be static content (for example, breadcrumbs) or interactive navigation when used with `LgCardNavigationTitleComponent`.
...
**Note: Heading level changes semantics only, while the visual title styling remains consistent.**

### LgCardNavigationTitleComponent

This is where the main title and link should be provided. It should be located inside the lg-card-header.
...
This is the footer layout section of the card component and is reserved for actions only, such as buttons, links, and menus.

### LgCardGroupComponent

A container that groups two or more cards, ensuring they are laid out correctly across breakpoints and share equal height. Applied as an attribute directive on a wrapping element:
...
It has no inputs. It should always be wrapped in an `lgCol="12"` container.
