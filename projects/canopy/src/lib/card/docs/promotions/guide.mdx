...
```js
  ...
  imports: [ ..., LgCardComponent, etc., LgMarginComponent, LgPaddingComponent, LgOrientationComponent ],
```
...
In the examples bellow, we use the Canopy Grid System to get the desired layout, together with the Orientation directive, and the component `LgCardHeroImageComponent` uses an image but it can be used with either an image or a pictogram.
