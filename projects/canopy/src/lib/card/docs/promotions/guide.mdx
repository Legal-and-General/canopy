import {Markdown, Meta} from '@storybook/addon-docs/blocks';
import CardHeroImageComponent from '../card-hero-image-component.md';
import Feedback from '../../../docs/common/feedback.md';
import { CustomBadge } from 'storybook-addon-tag-badges/manager-helpers';

<Meta title="Patterns/Promotions/Guide" tags={['updated']}/>

# Promotions

<span style={{ display: 'inline-flex', alignItems: 'baseline', gap: '0.25rem' }}><CustomBadge text="Updated" style={{ backgroundColor: '#caeedd', borderColor: '#caeedd' }} /><span class="small-text">for brand modernisation in Canopy{' '}<a href="https://github.com/Legal-and-General/canopy/releases/tag/v38.0.0" target="_blank" rel="noopener noreferrer">v38</a>.</span></span>

<p class="standfirst">Promotions are containers for content and actions on a specific topic, that use the Cards component and combine the Grid functionality to obtain a Magazine layout based on how many promotions you want to display.</p>

## Usage

They are used to group promotional content within a page.

Import the following components in your application:

```js
  import { LgCardComponent, LgMarginDirective, LgPaddingDirective, LgOrientationDirective } from '@legal-and-general/canopy';
```

and in your HTML, for a general promotion card with image:

```html
<lg-card
  lgMarginBottom="6"
  lgPadding="none"
  variant="promo"
  [lgOrientation]="{ sm: 'vertical', md: 'horizontal', lg: 'horizontal' }">
  <lg-card-hero-img [cover]="true" [src]="imageUrl">
  </lg-card-hero-img>
  <lg-card-header>
    <lg-card-title [headingLevel]="3">{{ title }}</lg-card-title>
  </lg-card-header>
  <lg-card-content>
    <p lgMarginBottom="6">{{ text }}</p>
    <a href="#">{{ buttonText }}</a>
  </lg-card-content>
</lg-card>
```

and in your HTML, for a general promotion card with icon:

```html
<lg-card
  lgMarginBottom="6"
  variant="promo"
  [lgOrientation]="{ sm: 'vertical', md: 'horizontal', lg: 'horizontal' }">
  <lg-card-hero-img>
    <lg-pictogram [name]="iconName" size="lg" [hasFill]="false"></lg-pictogram>
  </lg-card-hero-img>
  <lg-card-header>
    <lg-card-title [headingLevel]="3">{{ title }}</lg-card-title>
  </lg-card-header>
  <lg-card-content>
    <p lgMarginBottom="6">{{ text }}</p>
    <a href="#">{{ buttonText }}</a>
  </lg-card-content>
</lg-card>
```

## How it works

Canopy provides a number of patterns for a range of common scenarios, and we recommend you use those before creating something from scratch.

1. **Stand-alone Card** pattern layout.
2. **Two Cards** pattern layout.
3. **Three Cards** pattern layout.
4. **Magazine** pattern layout.

## Dos and Don'ts

### Do

1. **Do** keep the message succinct.
2. **Do** use equal amount of content across components for alignment.
3. **Do** use in line with the variants available.

## Promotions content and templates

As the promotion card is just a container for content, you can add whatever content you like.
In the examples below, we use the Canopy Grid System to get the desired layout, together with the Orientation directive, and the component `LgCardHeroImageComponent` uses an image but it can be used with either an image or a pictogram.

### Stand-alone card layout

```html
<div lgContainer>
  <div lgRow>
    <div lgColSm="12">
      <lg-card
        lgMarginBottom="6"
        variant="promo"
        [lgOrientation]="{ sm: 'vertical', md: 'horizontal', lg: 'horizontal' }">
        <lg-card-hero-img [cover]="true" [src]="imageUrl">
        </lg-card-hero-img>
        <lg-card-header>
          <lg-card-title [headingLevel]="3">{{ title }}</lg-card-title>
        </lg-card-header>
        <lg-card-content>
          <p lgMarginBottom="6">{{ text }}</p>
          <a href="#">{{ buttonText }}</a>
        </lg-card-content>
      </lg-card>
    </div>
  </div>
</div>
```

### Two Cards layout

```html
<div lgContainer>
  <div lgRow>
    <div lgColSm="12" lgColLg="6">
      <lg-card
        lgMarginBottom="6"
        lgPadding="none"
        variant="promo"
        [lgOrientation]="{ sm: 'vertical', md: 'horizontal', lg: 'vertical' }">
        <lg-card-hero-img [cover]="true" [src]="imageUrl">
        </lg-card-hero-img>
        <lg-card-header>
          <lg-card-title [headingLevel]="3">{{ title }}</lg-card-title>
        </lg-card-header>
        <lg-card-content>
          <p lgMarginBottom="6">{{ text }}</p>
          <a href="#">{{ buttonText }}</a>
        </lg-card-content>
      </lg-card>
    </div>
    <div lgColSm="12" lgColLg="6">
      <lg-card
        lgMarginBottom="6"
        lgPadding="none"
        variant="promo"
        [lgOrientation]="{ sm: 'vertical', md: 'horizontal', lg: 'vertical' }">
        <lg-card-hero-img [cover]="true" [src]="imageUrl">
        </lg-card-hero-img>
        <lg-card-header>
          <lg-card-title [headingLevel]="3">{{ title }}</lg-card-title>
        </lg-card-header>
        <lg-card-content>
          <p lgMarginBottom="6">{{ text }}</p>
          <a href="#">{{ buttonText }}</a>
        </lg-card-content>
      </lg-card>
    </div>
  </div>
</div>
```

### Three Cards layout

```html
<div lgContainer>
  <div lgRow>
    <div lgColSm="12" lgColLg="4">
      <lg-card
        lgMarginBottom="6"
        lgPadding="none"
        variant="promo"
        [lgOrientation]="{ sm: 'vertical', md: 'horizontal', lg: 'vertical' }">
        <lg-card-hero-img [cover]="true" [src]="imageUrl">
        </lg-card-hero-img>
        <lg-card-header>
          <lg-card-title [headingLevel]="3">{{ title }}</lg-card-title>
        </lg-card-header>
        <lg-card-content>
          <p lgMarginBottom="6">{{ text }}</p>
          <a href="#">{{ buttonText }}</a>
        </lg-card-content>
      </lg-card>
    </div>
    <div lgColSm="12" lgColLg="4">
      <lg-card
        lgMarginBottom="6"
        lgPadding="none"
        variant="promo"
        [lgOrientation]="{ sm: 'vertical', md: 'horizontal', lg: 'vertical' }">
        <lg-card-hero-img [cover]="true" [src]="imageUrl">
        </lg-card-hero-img>
        <lg-card-header>
          <lg-card-title [headingLevel]="3">{{ title }}</lg-card-title>
        </lg-card-header>
        <lg-card-content>
          <p lgMarginBottom="6">{{ text }}</p>
          <a href="#">{{ buttonText }}</a>
        </lg-card-content>
      </lg-card>
    </div>
    <div lgColSm="12" lgColLg="4">
      <lg-card
        lgMarginBottom="6"
        lgPadding="none"
        variant="promo"
        [lgOrientation]="{ sm: 'vertical', md: 'horizontal', lg: 'vertical' }">
        <lg-card-hero-img [cover]="true" [src]="imageUrl">
        </lg-card-hero-img>
        <lg-card-header>
          <lg-card-title [headingLevel]="3">{{ title }}</lg-card-title>
        </lg-card-header>
        <lg-card-content>
          <p lgMarginBottom="6">{{ text }}</p>
          <a href="#">{{ buttonText }}</a>
        </lg-card-content>
      </lg-card>
    </div>
  </div>
</div>
```

### Magazine layout

To achieve this layout add the `LgSeparatorComponent` to the imports.

```html
<div lgContainer>
  <div lgRow>
    <div lgCol="12">
      <lg-separator variant="dotted" lgMarginTop="none"></lg-separator>
    </div>
    <div lgColSm="12" lgColLg="4">
      <lg-card
        lgMarginBottom="6"
        lgPadding="none"
        variant="promo"
        [lgOrientation]="{ sm: 'vertical', md: 'horizontal', lg: 'vertical' }">
        <lg-card-hero-img [cover]="true" [src]="imageUrl">
        </lg-card-hero-img>
        <lg-card-header>
          <lg-card-title [headingLevel]="3">{{ title }}</lg-card-title>
        </lg-card-header>
        <lg-card-content>
          <p lgMarginBottom="6">{{ text }}</p>
          <a href="#">{{ buttonText }}</a>
        </lg-card-content>
      </lg-card>
    </div>
    <div lgColSm="12" lgColLg="4">
      <lg-card
        lgMarginBottom="6"
        lgPadding="none"
        variant="promo"
        [lgOrientation]="{ sm: 'vertical', md: 'horizontal', lg: 'vertical' }">
        <lg-card-hero-img [cover]="true" [src]="imageUrl">
        </lg-card-hero-img>
        <lg-card-header>
          <lg-card-title [headingLevel]="3">{{ title }}</lg-card-title>
        </lg-card-header>
        <lg-card-content>
          <p lgMarginBottom="6">{{ text }}</p>
          <a href="#">{{ buttonText }}</a>
        </lg-card-content>
      </lg-card>
    </div>
    <div lgColSm="12" lgColLg="4">
      <lg-card
        lgMarginBottom="6"
        lgPadding="none"
        variant="promo"
        [lgOrientation]="{ sm: 'vertical', md: 'horizontal', lg: 'vertical' }">
        <lg-card-hero-img [cover]="true" [src]="imageUrl">
        </lg-card-hero-img>
        <lg-card-header>
          <lg-card-title [headingLevel]="3">{{ title }}</lg-card-title>
        </lg-card-header>
        <lg-card-content>
          <p lgMarginBottom="6">{{ text }}</p>
          <a href="#">{{ buttonText }}</a>
        </lg-card-content>
      </lg-card>
    </div>
  </div>
  <div lgRow>
    <div lgCol="12">
      <lg-separator variant="dotted" lgMarginTop="none"></lg-separator>
    </div>
    <div lgColSm="12" lgColLg="6">
      <lg-card
        lgMarginBottom="6"
        variant="promo"
        [lgOrientation]="{ sm: 'vertical', md: 'horizontal', lg: 'vertical' }">
        <lg-card-hero-img>
          <lg-pictogram [name]="iconName" size="lg" [hasFill]="false"></lg-pictogram>
        </lg-card-hero-img>
        <lg-card-header>
          <lg-card-title [headingLevel]="3">{{ title }}</lg-card-title>
        </lg-card-header>
        <lg-card-content>
          <p lgMarginBottom="6">{{ text }}</p>
          <a href="#">{{ buttonText }}</a>
        </lg-card-content>
      </lg-card>
    </div>
    <div lgColSm="12" lgColLg="6">
      <lg-card
        lgMarginBottom="6"
        variant="promo"
        [lgOrientation]="{ sm: 'vertical', md: 'horizontal', lg: 'vertical' }">
        <lg-card-hero-img>
          <lg-pictogram [name]="iconName" size="lg" [hasFill]="false"></lg-pictogram>
        </lg-card-hero-img>
        <lg-card-header>
          <lg-card-title [headingLevel]="3">{{ title }}</lg-card-title>
        </lg-card-header>
        <lg-card-content>
          <p lgMarginBottom="6">{{ text }}</p>
          <a href="#">{{ buttonText }}</a>
        </lg-card-content>
      </lg-card>
    </div>
  </div>
</div>
```

## Additional development details

### LgCardComponent

The main body of the promotion card. It's composed by the following components:

* `LgCardHeroImageComponent`
* `LgCardContent`

#### Inputs

| Name      | Description                          | Type     | Default   | Required |
|-----------|--------------------------------------|----------|-----------|----------|
| `variant` | The variant of the card: `promo` | `string` | `default` | Yes      |

<Markdown>
  {CardHeroImageComponent}
</Markdown>

### LgCardContent

This is the secondary layout section of the card component, it is used to contain the cards main content and provides the inner padding of the card.

***

## Research on this component

This component hasn't been subject to any testing, but is based on best practice:

* <a href="https://uxplanet.org/best-practices-for-cards-fa45e3ad94dd" target="_blank">Best Practices for Cards</a>
* <a href="https://medium.com/nextux/design-better-cards-c0d12ab581c4" target="_blank">Design Better Cards</a>
* <a href="https://www.nngroup.com/articles/cards-component" target="_blank">Cards: UI-Component Definition</a>

## Related

Patterns which may perform a similar task, or which should be considered instead:

* [Cards](./?path=/docs/components-card-guide--docs)

<Markdown>
  {Feedback}
</Markdown>
