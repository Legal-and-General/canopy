...
<Meta title="Components/Button/Guide" parameters={{ viewMode: 'docs', previewTabs: { canvas: { hidden: true } } }} />
...
Buttons can be grouped together by using the ButtonGroupComponent – a container to group together two or more buttons and place them along a single line.
...
To see an example of the toggle button set up correctly inside a container, and toggling the visibility of a panel, see the [Filter Container story](./?path=/docs/components-filter-container--standard-filter-container).
...
- [Quick Action](./?path=/docs/components-quick-action--quick-action-button)
