import { Meta, Markdown, Source } from '@storybook/addon-docs/blocks';
import Feedback from '../../docs/common/feedback.md?raw';
import * as ButtonsGroupStories from './button-group.stories';

<Meta title="Components/Button/Guide"/>

# Button

<p class="standfirst">Buttons are interactive components that let a user initiate an action.</p>

![Button component example](docs/button/example.png)

## Usage

Use a button when you want to give the user a clear signpost as to what to do next. Examples of this are initiating a journey, submitting a form, or getting help.

Import the button component in your application:

```js
import { LgButtonComponent } from '@legal-and-general/canopy';
```

and in your HTML:

```html
<button lg-button type="button" priority="primary">Button</button>
```

or

```html
<a lg-button href="#" priority="primary">Link</a>
```

## Priority

![Button component - button styles](docs/button/styles.png)

### Primary

Primary buttons should only be used once per page. Use them to clearly signpost the action the customer should take next.

```html
<button lg-button priority="primary">
  Click me
</button>
```

### Secondary

Secondary buttons can be used many times per page. Use them for secondary actions where you wouldn't use a link.

```html
<button lg-button priority="secondary">
  Click me
</button>
```

### Button styling with colour modes

Button styles automatically adapt to the colour mode and theme set by their container element using the `lgColour` directive. This ensures buttons display appropriately for different background colors.

```html
<lg-card lgColour="green" lgColourTheme="bold">
  <lg-card-content>
    <button lg-button priority="primary">Action</button>
  </lg-card-content>
</lg-card>
```

The button will automatically apply the appropriate style based on the container's colour mode and theme, without needing separate variant inputs.

### Button content

Buttons can contain the following content types:

![Button component - types of button](docs/button/types.png)

1. Label only
2. Icon + label
3. Label + icon
4. Icon-only
5. Loading

#### Buttons with labels and icons

##### Icon on the right

An **icon component** can be projected into the button. By default, icons are positioned on the right side of the text.

```html
<button lg-button type="button">
  Click me
  <lg-icon name="chevron-right" />
</button>
```

##### Icon on the left

Use the `leftIcon` boolean input to display an arrow-left icon on the left side of the button. When `leftIcon` is set to `true`, it takes precedence and will display the arrow-left icon regardless of any other icons in the markup.

```html
<button lg-button type="button" leftIcon="true">
  Back
</button>
```

**Note:** The `leftIcon` input specifically displays an arrow-left icon and is intended for navigation purposes. You cannot use a custom icon with this input.

#### Icon-only buttons

You can create an **icon only** button by setting iconButton to true. You must still include text content, which will be visually hidden, but detectable by screen readers.

```html
<button lg-button type="button" iconButton="true" >
  Add item
  <lg-icon name="add" />
</button>
```

## Migration notes

### Removal of `iconPosition` input

The `iconPosition` input has been removed. The button now uses a `leftIcon` boolean input which displays an arrow-left icon on the left side of the button for navigation purposes.

**Before:**
```html
<button lg-button iconPosition="left">
  Back
  <lg-icon name="arrow-left" />
</button>
```

**After:**

For navigation with arrow-left icon:
```html
<button lg-button leftIcon="true">
  Back
</button>
```

For custom icons on the right (default behavior):
```html
<button lg-button>
  Add item
  <lg-icon name="add" />
</button>
```

**Important:** The `leftIcon` input specifically displays an arrow-left icon. Buttons can no longer have two icons simultaneously (e.g., icons on both left and right sides). If you were using the `first` and `second` attributes with two icons, you'll need to choose which single icon to display.

### Removal of `buttonSize` input

The `buttonSize` input has been removed. Buttons now have a single, consistent size across all implementations. This simplifies the button API and ensures a more uniform visual appearance.

**Before:**
```html
<button lg-button buttonSize="sm">
  Small button
</button>
```

**After:**
```html
<button lg-button>
  Button
</button>
```

If you require custom sizing for specific use cases, consider using CSS utilities or custom styling.

### Removal of `primary-light` and `secondary-light` variants

The `primary-light` and `secondary-light` button priority variants have been removed. Button styling now automatically adapts based on the colour mode and theme set by the container element using the `lgColour` directive.

**Before:**
```html
<button lg-button priority="primary-light">
  Click me
</button>
```

**After:**
```html
<lg-card lgColour="green" lgColourTheme="bold">
  <lg-card-content>
    <button lg-button priority="primary">
      Click me
    </button>
  </lg-card-content>
</lg-card>
```

The button will automatically apply the appropriate style based on the container's colour mode and theme. Simply use `priority="primary"` or `priority="secondary"` and the button will adapt to its context.
![Button component - what you shouldn't do](docs/button/dont.png)

1. **Don't** use a button priority that contrasts poorly with the background.
2. **Don't** use more than one primary button per page and don't use too many buttons in close proximity.
3. **Don't** wrap text labels.
4. **Don't** confuse priorities by using multiple primary CTA's at the same time.
5. **Don't** use overly wordy labels.
6. **Don't** centre align buttons.
7. **Don't** right align buttons.
8. **Don't** use uninformative labels.

## Additional development details

### LgButtonComponent inputs

|                | Description                                                                                                                                     | Type                      | Default        | Required |
|----------------|-------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------|----------------|----------|
| `priority`      | The priority of button: `primary`,`secondary`, `link`.                                   | `string`                  | `primary` | No       |
| `fullWidth`    | If the button has to span full width or not. For 'sm' and 'md' sized screens, the button will always be full width and this input has no affect | `boolean`                 | `false`        | No       |
| `disabled`     | Programmatically disable the button via this property                                                                                           | `boolean`                 | `false`        | No       |
| `loading`      | If the button shows a loading spinner and is also disabled                                                                                      | `boolean`                 | `false`        | No       |
| `leftIcon`     | Display arrow-left icon on the left side of the button                                                                                          | `boolean`                 | `false`        | No       |
| `rightIcon`    | Icon name to display on the right side of the button. Only one icon can be displayed at a time                                                 | `IconName \| null`        | `null`         | No       |
| `iconButton`   | The button displays an icon only                                                                                                                | `boolean`                 | `false`        | No       |

### Button group

Buttons can be grouped together by using the `LgButtonGroupComponent` – a container to group together two or more buttons and place them along a single line.

Useful for forms where a 'Submit' and 'Cancel' button are placed side by side, for example.

<Source of={ButtonsGroupStories.StandardButtonGroup} />

### Button toggle directive

A button can be used specifically as a toggle, for example when opening and closing a filter panel. In this case the applied styles are slightly different from a regular button and you **must** use a button tag for accessibility reasons - if you add it to a link or any other element, it will throw an error.

*This feature should only be used as part of a pattern. E.g. toggling the visibility of a filter panel.*

```html
<button lg-button lgButtonToggle type="button" priority="primary">Button</button>
```

then in the wrapper component set the `id` and `ariaControls` of the toggle and use the `toggleActive` output of the directive to set the panel state:

```js
@ContentChild(forwardRef(() => LgButtonToggleDirective)) buttonToggle: LgButtonToggleDirective;

...

ngAfterContentInit(): void {
  this.buttonToggle.id = '<the-unique-id-of-the-toggle>';

  this.buttonToggle.ariaControls = '<the-unique-id-of-the-panel-to-toggle>';

  this.subscription = this.buttonToggle.toggleActive.subscribe(isActive => {
    // set the active state of the panel e.g.:
    // this.myPanelComponent.isActive = isActive;
  });
}
```

To see an example of the toggle button set up correctly inside a container, and toggling the visibility of a panel, see the [filter container story](./?path=/docs/patterns-filter-container-guide--docs).

***

## Research on this pattern

This component is based on best practice.

States have been updated to be more accessible based on external accessibility testing.

## Related

* [Quick action](./?path=/docs/components-quick-action-guide--docs)
* [Link](./?path=/docs/components-quick-action-guide--docs)

<Markdown>
  {Feedback}
</Markdown>
