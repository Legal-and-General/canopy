...
Using the `LgHeroCardHeaderComponent` together with `LgBreadcrumbComponent`, some breadcrumbs can be easily added to the hero, [breadcrumb component](./?path=/docs/components-breadcrumb-guide--docs).
