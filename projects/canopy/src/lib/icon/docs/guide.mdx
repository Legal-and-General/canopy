...
<Unstyled>
  <blockquote>
    <p>
      `currentColor` acts like a variable for the current value of the `color` property
      on the element. And the Cascading part of CSS is still effective, so if there's
      no defined `color` property on an element, the cascade will determine the value
      of `currentColor`.
    </p>
    <cite>
      <a href="https://www.digitalocean.com/community/tutorials/css-currentcolor" target="_blank">
