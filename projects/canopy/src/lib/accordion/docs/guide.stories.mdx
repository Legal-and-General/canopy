...
If needed visually, an icon can be set for each accordion item heading. The icon must match an `lgIcon` asset which has to be to be registered with the `LgIconRegistry.registerIcons` method.
