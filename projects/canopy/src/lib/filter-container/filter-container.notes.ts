...
More information on how to use the \`lgButtonToggle\` directive is available in the <a href="./?path=/docs/components-button-guide--page">button documentation</a>. Note that there is no need to set the \`id\` and \`ariaControls\` properties in this case as they are already handled by the \`LgFilterContainerComponent\`.
`;
