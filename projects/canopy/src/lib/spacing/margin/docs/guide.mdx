...
> `lgMarginRightNoneAt="sm"` applies from `sm` to just before `md`
>
> `lgMarginRightNoneAt="md"` applies from `md` to just before `lg`
>
> `lgMarginRightNoneAt="xxl"` applies from `xxl` and up (no upper limit)
...
**Before:**
```html
<lg-card [lgMarginRight]="{ sm: 'none', lg: '4' }"></lg-card>
```

**After:**
```html
<lg-card lgMarginRight="4" lgMarginRightNoneAt="sm" lgMarginRightNoneAt="md"></lg-card>
```
