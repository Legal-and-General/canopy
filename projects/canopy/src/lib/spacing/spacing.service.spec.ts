...
describe('SpacingService', () => {
  ...
  describe('createNoneAtClass method', () => {
    it('should return empty array when breakpoint is null', () => {
      expect(spacingService.createNoneAtClass(null, 'margin')).toEqual([]);
    });

    it('should return empty array when breakpoint is undefined', () => {
      expect(spacingService.createNoneAtClass(undefined, 'margin')).toEqual([]);
    });

    it('should return empty array when breakpoint is empty array', () => {
      expect(spacingService.createNoneAtClass([], 'margin')).toEqual([]);
    });
    ...
    it('should correctly replace hyphens with double underscores for all margin sides', () => {
      [ 'margin-top', 'margin-right', 'margin-bottom', 'margin-left' ].forEach(side => {
        const classNames = spacingService.createNoneAtClass('md', side);

        expect(classNames).toHaveLength(1);
        expect(classNames[0]).toContain('__');
        expect(classNames[0]).not.toContain(`lg-${side}`);
        expect(classNames[0]).toEqual(`lg-${side.replace('-', '__')}--md--none`);
      });
    });
    ...
    it('should correctly replace hyphens with double underscores for all padding sides', () => {
      [ 'padding-top', 'padding-right', 'padding-bottom', 'padding-left' ].forEach(
        side => {
          const classNames = spacingService.createNoneAtClass('lg', side);

          expect(classNames).toHaveLength(1);
          expect(classNames[0]).toContain('__');
          expect(classNames[0]).not.toContain(`lg-${side}`);
          expect(classNames[0]).toEqual(`lg-${side.replace('-', '__')}--lg--none`);
        },
      );
    });

    it('should handle array of breakpoints', () => {
      expect(spacingService.createNoneAtClass([ 'sm', 'lg' ], 'margin')).toEqual([
        'lg-margin--sm--none',
        'lg-margin--lg--none',
      ]);

      expect(spacingService.createNoneAtClass([ 'md', 'xl', 'xxl' ], 'padding')).toEqual([
        'lg-padding--md--none',
        'lg-padding--xl--none',
        'lg-padding--xxl--none',
      ]);
    ...
    it('should handle array of breakpoints for specific sides', () => {
      expect(spacingService.createNoneAtClass([ 'sm', 'lg' ], 'margin-top')).toEqual([
        'lg-margin__top--sm--none',
        'lg-margin__top--lg--none',
      ]);

      expect(spacingService.createNoneAtClass([ 'md', 'xl' ], 'padding-right')).toEqual([
        'lg-padding__right--md--none',
        'lg-padding__right--xl--none',
      ]);
    });

    it('should filter duplicate breakpoints in arrays', () => {
      expect(spacingService.createNoneAtClass([ 'sm', 'sm', 'lg' ], 'margin')).toEqual([
        'lg-margin--sm--none',
        'lg-margin--lg--none',
      ]);

      expect(
        spacingService.createNoneAtClass([ 'md', 'xl', 'md', 'xl' ], 'padding'),
      ).toEqual([ 'lg-padding--md--none', 'lg-padding--xl--none' ]);
    });
  });
});
